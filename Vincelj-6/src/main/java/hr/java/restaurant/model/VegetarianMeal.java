...
public non-sealed class VegetarianMeal extends Meal implements Vegetarian, Serializable {
    private static Logger log = LoggerFactory.getLogger(VegetarianMeal.class);

    private boolean containsDairy;
    private boolean containsEggs;

    public VegetarianMeal(Long id, String name, Category category, Set<Ingredient> ingredients, BigDecimal price, Integer calories, boolean containsDairy, boolean containsEggs) {
    ...
    public static void inputVegetarianMeal(List <Meal> vegetarianMeals, List <Category> categories, List<Ingredient> ingredients, Scanner scanner) {
        for (int i = 0; i < vegetarianMeals.size(); i++) {
            Boolean isValid;
            String name;
            do {
                isValid = true;
            ...
            System.out.print("Does the vegetarian meal contain dairy? (true/false): ");
            boolean containsDairy = InputValidator.validateBoolean(scanner, Messages.INVALID_BOOLEAN_INPUT);

            System.out.print("Does the vegetarian meal contain eggs? (true/false): ");
            boolean containsEggs = InputValidator.validateBoolean(scanner, Messages.INVALID_BOOLEAN_INPUT);

            vegetarianMeals.add(new VegetarianMeal(Long.valueOf(i + 1), name, category, availableIngredients, price, calories, containsDairy, containsEggs));
        }
    }
}
